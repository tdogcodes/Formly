...
import { startChat } from "@/lib/googleAi";
...
const AIAssistanceBtn = () => {
  ...
  const GenerateFormQuestionsWithAI = async () => {
    ...
    const responseText = await startChat(PROMPT);
  ...
  const [count, setCount] = useState(() => {
    const cached = localStorage.getItem("ai-credit-count");
    return cached ? Number(cached) : 3;
  });

  React.useEffect(() => {
    localStorage.setItem("ai-credit-count", String(count));
  }, [count]);
          ...
          >
            ...
            <div className="mt-[22px]">
              <Textarea
                ...
                focus:ring-primary"
                placeholder={count > 1 ? `You have ${count} Credits, describe the form or questions you want to generate with AI...` : "You have no credits..." }
                spellCheck="false"
                onChange={(e) => {
            ...
            >
              ...
              <Button
                ...
                onClick={() => {
                    setCount(count - 1);
                    return GenerateFormQuestionsWithAI()
                }}
              >
                <Sparkles />
                Generate
